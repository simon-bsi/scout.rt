 ...
 */
import {
  Column, HybridManager, HybridManagerAdapter, InitModelOf, JsPageHelper, Outline, OutlineAdapter, Page, PageParamDo, PageWithTable, RemoteEvent, scout, styles, Table, TableRow, TreeNodeModel, typeName, UuidPool
} from '../../../../../src';
...
describe('JsPageHelper', () => {
  ...
  describe('rowsInserted', () => {
    ...
    it('triggers nodeChanged event for linked pages', async () => {
      outline.insertNode({
        ...outlineSpecHelper.createModelNode(null, 'bar'),
        objectType: MyPageWithTable,
        childNodes: [
      ...
      const nodeChangedPages: Page[] = [];
      outline.on('nodeChanged', e => nodeChangedPages.push(e.node as Page));

      await myPageWithTable.ensureLoadChildren();

      const [page1, page2] = helper.findChildPages(['1', '2']);
      expect(nodeChangedPages).toEqual([page1, page2]);
    });
  });
  ...
  describe('callLoadChildPages', () => {
    ...
    it('loads child pages using page params', async () => {
      ...
      sendQueuedAjaxCalls();
      expect(mostRecentJsonRequest()).toContainEvents(new RemoteEvent(hybridManagerAdapter.id, 'hybridAction', {
