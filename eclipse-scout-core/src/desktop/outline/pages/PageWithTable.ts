...
import {
  ...
  PropertyChangeEvent, scout, SearchFormTableControl, Status, Table, TableAllRowsDeletedEvent, TableControl, TableMaxResultsHelper, TableReloadEvent, TableReloadReason, TableRow, TableRowActionEvent, TableRowOrderChangedEvent,
...
export class PageWithTable extends Page implements PageWithTableModel {
  ...
  protected override _initDetailTable(table: Table) {
    ...
    if (this.outline.compact) {
      ...
      // hide all table controls except
  ...
  protected _onTableControlsChange(e: PropertyChangeEvent<TableControl[]>) {
    ...
    arrays.ensure(e.oldValue)
      .filter<SearchFormTableControl>((tableControl: TableControl): tableControl is SearchFormTableControl => tableControl instanceof SearchFormTableControl)
      .forEach(searchFormTableControl => {
    ...
    const searchFormTableControl = this.getSearchFormTableControl();
    searchFormTableControl?.on('search', this._searchFormTableControlSearchHandler);
    searchFormTableControl?.on('reset', this._searchFormTableControlResetHandler);
  }
