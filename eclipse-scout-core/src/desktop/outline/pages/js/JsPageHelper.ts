...
import {
  arrays, dataObjects, HybridActionContextElement, HybridActionContextElements, HybridManager, InitModelOf, LoadChildPagesHybridActionDo, objects, ObjectWithType, Page, PageParamDo, PageWithTable, scout, SomeRequired, styles, Table,
  TableRowsInsertedEvent, TreeAllChildNodesDeletedEvent, TreeNodesDeletedEvent, TreeNodesInsertedEvent, typeName
} from '../../../../index';
 ...
 * The child pages are created by the Java pendant of the JsPage (see org.eclipse.scout.rt.client.ui.desktop.outline.pages.js.AbstractJsPage.createChildPage) when {@link #callLoadChildPages} is called.
 ...
 * <ul>
 *   <li> Call and await {@link #callLoadChildPages} in {@link PageWithTable#_loadTableData} using ids or {@link PageParamDo}s created from the loaded data.
 *   <li> Implement {@link PageWithTable#_createChildPage} and return the result of {@link #findChildPage} using an id or {@link PageParamDo} created for the given {@link TableRow}.
 * </ul>
 *
 * To create child pages for a {@link PageWithNodes}:
 * <ul>
 *   <li> Implement {@link PageWithNodes#_createChildPages} and call {@link #callLoadChildPages} using ids or {@link PageParamDo}s.
 *   <li> After the {@link Promise} is resolved use the same ids or {@link PageParamDo}s to call {@link #findChildPages} and return the resulting child pages.
 * </ul>
 */
export class JsPageHelper implements JsPageHelperModel, ObjectWithType {
  ...
  protected _installListeners() {
    this.page.outline.on('nodesDeleted', this._nodesDeletedHandler);
    this.page.outline.on('allChildNodesDeleted', this._allChildNodesDeletedHandler);
  }

  protected _uninstallListeners() {
    this.page.outline.off('nodesDeleted', this._nodesDeletedHandler);
    this.page.outline.off('allChildNodesDeleted', this._allChildNodesDeletedHandler);
  }
  ...
  protected _installPageWithTableListeners() {
    ...
    this.page.outline.on('nodesInserted', this._nodesInsertedHandler);

    // detailTable may change -> install for current and listen on propertyChange
    this._installPageWithTableTableListeners(this.page?.detailTable);
    this.page.on('propertyChange:detailTable', e => {
      this._uninstallPageWithTableTableListeners(e.oldValue);
      this._installPageWithTableTableListeners(e.newValue);
    });
  }
  ...
  protected _uninstallPageWithTableListeners() {
    ...
    this.page.outline.off('nodesInserted', this._nodesInsertedHandler);
    this._uninstallPageWithTableTableListeners(this.page?.detailTable);
  }
  ...
  protected _onNodesInserted(event: TreeNodesInsertedEvent) {
    ...
    for (const childPage of event.nodes) {
      childPage.setCssClass(styles.toggleCssClass(childPage.cssClass, 'js-page-child-page-loading', !this.page.childrenLoaded));
      childPage._decorate();
    }
  }

  protected _installPageWithTableTableListeners(table: Table) {
    if (!table) {
      return;
    }
    table.on('rowsInserted', this._tableRowsInsertHandler);
  }

  protected _uninstallPageWithTableTableListeners(table: Table) {
    if (!table) {
      return;
    }
    table.off('rowsInserted', this._tableRowsInsertHandler);
  }
  ...
  protected _onTableRowsInserted(event: TableRowsInsertedEvent) {
    ...
    // Trigger a nodeChanged event in order to send summary information from parent PageWithTable to the UI server.
    ...
    for (const page of pages) {
      this.page.outline.trigger('nodeChanged', {node: page});
    }
  }

  /**
   * Calls loadChildPages on the UI server for the given ids or {@link PageParamDo}s. The pages are created by the Java pendant of the JsPage (see org.eclipse.scout.rt.client.ui.desktop.outline.pages.js.AbstractJsPage.createChildPage).
   ...
   * @param replace Whether all child pages shall be replaced. If not given the child pages will always be replaced except for the first call in order to keep existing child pages after a browser reload.
   */
  async callLoadChildPages(idsOrPageParams: string | PageParamDo | (string | PageParamDo)[], replace?: boolean): Promise<void> {
    // ensure pageParams and replace flag
    const pageParams = arrays.ensure(idsOrPageParams)
    ...
    await hybridManager.callActionAndWaitWithContext('scout.LoadChildPages',
      ...
      scout.create(HybridActionContextElements)
        .withElement('page', HybridActionContextElement.of(this.page.outline, this.page))
    );
  ...
  findChildPages(idsOrPageParams: string | PageParamDo | (string | PageParamDo)[]): Page[] {
    ...
    if (!idsOrPageParamsArray.length) {
      return [];
    }
  ...
  protected _addChildPagesToIdMap(childPages?: JsPageChildPage[]) {
    ...
    for (const childPage of childPages) {
      // extract and assert id
      const id = this._extractChildPageId(childPage);
      scout.assertValue(id, `${childPage} does not have a child page param.`);
      ...
      this.page.outline.hideNode(childPage, false);
    }
  ...
  /**
   * Extracts the child page id from the given {@link JsPageChildPage}.
   */
  protected _extractChildPageId(childPage?: JsPageChildPage): string {
    // extract child page param
    const pageParam = this._extractChildPageParam(childPage);
    if (!pageParam) {
      return null;
    }
  ...
  /**
   * Extracts the child page param from the given {@link JsPageChildPage}.
   */
  protected _extractChildPageParam(childPage?: JsPageChildPage): PageParamDo {
