...
public class HttpServiceTunnel {
   ...
   */
  public void cancel(ServiceTunnelRequest serviceRequest) {
    try {
      final String sessionId = serviceRequest.getSessionId();
      if (sessionId == null) {
      ...
      final Object[] serviceArgs = {serviceRequest.getRequestSequence()};
      ServiceTunnelRequest request = createRequest(IRunMonitorCancelService.class, serviceMethod, serviceArgs);
      request.setClientNodeId(serviceRequest.getClientNodeId());
      request.setSessionId(sessionId);
      request.setUserAgent(serviceRequest.getUserAgent());
      ...
      LOG.warn("Failed to cancel server processing [requestSequence={}]", serviceRequest.getRequestSequence(), e);
    }
  }
